...
export function transformerContainer(): ShikiTransformer {
  return {
    ...
    root(node) {
      ...
      if (!pre) {
        throw new Error(`[${name}] Expected a <pre> element in the root node but got ${JSON.stringify(node)}`);
      }
      ...
      if (title) {
        children.unshift({
          ...
          properties: {
            class:
              "hidden border-b border-solid border-b-(--sl-color-gray-5) bg-(--sl-color-bg) px-4 py-2 font-mono text-sm text-(--sl-color-gray-3) [div[data-celestia-code-container][data-celestia-code-title]_&]:block",
          },
          children: [
            {
              type: "text",
      ...
      node.children = [
        {
          ...
          properties: {
            ...
            class:
              "not-content relative overflow-hidden [&_pre]:relative [&_celestia-code-copy-button]:opacity-0 [&:hover_celestia-code-copy-button]:opacity-100 border border-solid border-(--sl-color-gray-5) rounded-md",
          },
          children: [
            ...children,
            {
              type: "element",
              tagName: "celestia-code-copy-button",
              properties: {
                type: "button",
